...
// src/Command/ParseUserDataFromCSV2.php
...
#[AsCommand(
    name: 'app:parse-user-data-from-csv2',
    description: 'Parse User data from CSV and show total number of User and their avarage age.',
    hidden: false,
    aliases: ['app:parse-user-data2']
)]
class ParseUserDataFromCSV2 extends Command
{
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if($result->type == 'ok'){
            $totalUser = count($result->users);
            $totalAge = array_sum(array_column($result->users, 'age'));
            $ageAvarage = $totalUser ? round($totalAge/$totalUser) : 0;

            $output->writeln("Total Users: ".$totalUser);
            $output->writeln("Avarage Age: ".$ageAvarage);
